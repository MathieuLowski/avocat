...
router.post("/signup", (req, res)=>{
    ...
    .then((savedUser)=>{
        if(savedUser){
            return res.status(422).json({erro:"User already exists with that email"});
        }
    })
...
router.post("/signin", (req, res)=>{
    ...
    if(!email || !password){
        return res.status(422).json({status:422,error:"Please add email or password"});
    }
    User.findOne({email:email})
    .then((savedUser)=>{
        if(!savedUser){
            return res.status(422).json({status:422, message:"Incorrect email or password"});
        }
